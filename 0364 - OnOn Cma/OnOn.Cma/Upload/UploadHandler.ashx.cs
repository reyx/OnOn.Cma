...
namespace jQuery_File_Upload.MVC3.Upload
{
    ...
    public class UploadHandler : IHttpHandler
    {
        ...
        private void ListCurrentFiles(HttpContext context)
        {
            ...
            if (!string.IsNullOrWhiteSpace(context.Request.Params["start"]))
            {
                start = int.Parse(context.Request.Params["start"]);
                total = int.Parse(context.Request.Params["total"]);
            }

            string jsonObj = js.Serialize(new { max = files.Length, entries = files.Skip(start).Take(total) });
